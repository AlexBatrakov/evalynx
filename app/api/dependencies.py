...
def get_supported_runners(request: Request) -> set[str]:
    return set(request.app.state.runner_registry)
...
def get_run_service(
    ...
    supported_runners: Annotated[set[str], Depends(get_supported_runners)],
) -> RunService:
    return RunService(
        session=session,
        run_queue=run_queue,
